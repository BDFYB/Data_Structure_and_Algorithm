...
void bubble_sort(vector<int>& inputs) {
    ...
    for (int outer_c = 0; outer_c < inputs.size() - 1; outer_c++) {
        outer_c = outer_c > pos ? outer_c : pos;
        loop_count += 1;
        ...
        for (int inner_c2 = outer_c + 1; inner_c2 < inputs.size() - 1; inner_c2++) {
            if (inputs[inner_c2 - 1] > inputs[inner_c2]) {
                swap(inputs[inner_c2 - 1], inputs[inner_c2]);
            }
        }
    }
